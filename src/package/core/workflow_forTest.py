...
def evaluate_items_node(state: WorkflowState) -> WorkflowState:
    ...
    irt_repair_mode = state.get("irt_repair_mode", False)
    if irt_repair_mode:
        items_by_trait = state.get("irt_repair_trait_name", 0)
        prompt = format_all_items_prompt({"特质": {items_by_trait: items}})
    else:
...
def convert_to_CVI_node(state: WorkflowState) -> WorkflowState:
    ...
    print(f"--- 跳过CVI检查，直接保存所有生成的题目 ---")
    try:
        # 跳过CVI计算，直接将所有生成的题目当作通过
        passed_items = generated_items.copy()
        low_cvi_items = []
...
def analysis_node(state: WorkflowState) -> WorkflowState:
    print("\n--- 开始执行CITC分析 ---")
    try:
        # 跳过CITC计算，直接创建空的DataFrame并标记QN_3为坏题
        project_root = get_project_root()
        output_dir = project_root / "output" / "CITC_analysis"
        output_dir.mkdir(parents=True, exist_ok=True)
        
        # 创建空的citc_df，只包含QN_3，CITC分数为0.2
        import pandas as pd
        bad_qid = "QN_3"
        ...
        print(f"📄 已将题目 {bad_qid} 的CITC分数设置为 0.2（不合格）")
...
def accumulator_node(state: WorkflowState) -> WorkflowState:
    ...
    new_batch_count = 5
    if irt_repair_mode:
        print(f"🔧 [归档-修复] 修复题目已入库，批次保持: {new_batch_count}")
    else:
        print(f"\n [归档完成] 批次: {new_batch_count}")
